defmodule BiblioMaterializer.DataCite do
  @moduledoc """
  ...
  defp get_with_retry(url, headers, queue) do
    BiblioMaterializer.DataCiteQueue.await_slot(queue)
    ...
    case Req.get(url, [headers: headers, retry: &transient?/2] ++ @req_options) do
      ...
      {:ok, %Req.Response{status: 429} = resp} ->
        BiblioMaterializer.DataCiteQueue.backoff(queue, rate_limit_cooldown(resp))
