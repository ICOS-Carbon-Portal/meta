defmodule BiblioMaterializer.Subject do
  @moduledoc """
  ...
  The results are seeded into the shared [`Cache`](`BiblioMaterializer.Cache`)
  under `{group, subject_uri}` keys, which is where `BiblioMaterializer.Reader`
  and `BiblioMaterializer.Licence` look for them, so a batch is invisible to
