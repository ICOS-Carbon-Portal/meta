...
object HierarchicalBitmap{
	...
	final case class EqualsFilter[+K](key: K) extends FilterRequest[K]
	final case class MinFilter[+K](min: K, inclusive: Boolean) extends FilterRequest[K]
	final case class MaxFilter[+K](max: K, inclusive: Boolean) extends FilterRequest[K]
	final case class IntervalFilter[+K](from: MinFilter[K], to: MaxFilter[K]) extends FilterRequest[K]
