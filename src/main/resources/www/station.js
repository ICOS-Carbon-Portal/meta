...
function getLmUrl(layer) {
  return (
    "//api.lantmateriet.se/open/topowebb-ccby/v1/wmts/token/a39d27b8-9fd6-3770-84cb-2589b772b8ca/1.0.0/" +
    layer +
...
function getBaseMaps(maxZoom) {
  ...
  var topoTonedLM = L.tileLayer(
    window.location.protocol + getLmUrl("topowebb_nedtonad"),
    {
      maxNativeZoom: 14,
    }
  );
...
function getStationLocations(stationUrl) {
  return getJson(stationUrl).then(function (result) {
    try {
      const ownCoverage = [];
      if (result.location)
        ownCoverage.push({
      ...
      const sitesCoverage =
        result.specificInfo._type === "sites"
      ...
      try {
        if (req.readyState === 4) {
          if (req.status < 400 && req.status >= 200) {
            if (req.responseType === "json") resolve(req.response);
