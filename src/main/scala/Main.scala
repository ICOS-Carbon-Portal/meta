...
object Main extends App with CpmetaJsonProtocol{
	...
	val optIndexDataFut: Future[Option[IndexData]] =
		...
		val repositoryIsRemote = config.remoteRdfRepository.isDefined
		if(recreateIndex && !repositoryIsRemote) IndexHandler.dropStorage()
		if(repositoryIsRemote || recreateIndex || conf.disableCpIndex) Future.successful(None)
		else {
