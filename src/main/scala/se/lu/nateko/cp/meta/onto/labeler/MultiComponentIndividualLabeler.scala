...
import se.lu.nateko.cp.meta.instanceserver.TriplestoreConnection.{TS, getValues}
...
class MultiComponentIndividualLabeler(
	...
	private val compMakers: Seq[IRI => TS ?=> String] = components.map{
		case DataPropComponent(prop) => getComponent(prop)
		case ObjectPropComponent(prop) => getComponent(prop)
		case ConstantComponent(value) => (_: IRI) => (_: TS) ?=> value
	}
	...
	private def getComponent(propIri: OWLIRI)(instUri: IRI)(using TS): String =
	...
	private def getComponent(prop: OWLObjectProperty)(instUri: IRI)(using TS): String =
	...
	private def toUri(prop: OWLIRI)(using conn: TS): IRI =
