...
import se.lu.nateko.cp.meta.HttpRdfStoreConfig

import java.io.StringWriter
import scala.concurrent.{ExecutionContext, Future}
import scala.concurrent.duration.DurationInt

class HttpRdfStoreClient(val config: HttpRdfStoreConfig)(using system: ActorSystem, mat: Materializer):
		...
		http.singleRequest(request).flatMap: resp =>
			if resp.status.isSuccess then
				...
				resp.entity.toStrict(5.seconds).flatMap: strict =>
					Future.failed(Exception(s"SPARQL update failed with ${resp.status}: ${strict.data.utf8String}"))
	...
	def graphStoreRemove(graphUri: String, statements: Iterable[Statement]): Future[Done] =
		val ntriples = statementsToNTriples(statements)
		sparqlUpdate(s"DELETE DATA { GRAPH <$graphUri> {\n$ntriples} }")
	...
	private def graphStoreRequest(method: HttpMethod, graphUri: String, body: String, accept404: Boolean = false): Future[Done] =
		...
		http.singleRequest(request).flatMap: resp =>
			if resp.status.isSuccess || (accept404 && resp.status.intValue == 404) then
				...
				resp.entity.toStrict(5.seconds).flatMap: strict =>
					Future.failed(Exception(s"Graph store request failed with ${resp.status}: ${strict.data.utf8String}"))
