...
class CpEvaluationStrategyFactory(
	...
	override def createEvaluationStrategy(
		...
		new DefaultEvaluationStrategy(tripleSrc, dataSet, fedResolver, 0, stats) { defaultStrat =>
			setOptimizerPipeline(CpQueryOptimizerPipeline(defaultStrat, tripleSrc, stats))
