...
class PageContentMarshalling(
	...
	def staticObjectMarshaller(using Envri, EnvriConfig): ToResponseMarshaller[() => Validated[StaticObject]] =
		makeLandingPageMarshaller(
	...
	private def makeLandingPageMarshaller[T: JsonWriter, P <: LandingPage](
		assemble: (T, ErrorList) => Future[P],
		notFoundPage: ErrorList => Html,
	)(using Envri, EnvriConfig): ToResponseMarshaller[() => Validated[T]] =
