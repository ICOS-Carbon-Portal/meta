...
class DoiGeoLocationCreatorTests extends AnyFunSpec:
	describe("DoiGeoLocationCreator"):
		def convertStringsToJTS(geomStrings: Seq[String]): Seq[Geometry] = 
		...
		it("createHulls from ecosystem data"):
			...
			val expectedGeomStrings = Vector(
			...
			val expectedGeometries = convertStringsToJTS(expectedGeomStrings)
