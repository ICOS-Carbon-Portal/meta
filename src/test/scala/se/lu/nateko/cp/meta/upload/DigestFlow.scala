...
object DigestFlow{
	...
	def md5: Flow[ByteString, ByteString, Future[Md5Sum]] = forDigest("MD5")(bytes => new Md5Sum(bytes))
	def sha1: Flow[ByteString, ByteString, Future[Array[Byte]]] = forDigest("SHA-1")(identity)
	def sha256: Flow[ByteString, ByteString, Future[Sha256Sum]] = forDigest("SHA-256")(bytes => new Sha256Sum(bytes))
