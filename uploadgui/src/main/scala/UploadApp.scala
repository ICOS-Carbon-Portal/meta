...
object UploadApp {
	...
	private def createDoi(uri: URI): Unit = {
		whenDone{
			...
			showAlert(s"Draft DOI created: ${doi}. <a class='alert-link' target='_blank' href='https://doi.icos-cp.eu/doi/${doi}'>Edit or submit for publication</a>", "alert alert-success")
		}).onComplete {
